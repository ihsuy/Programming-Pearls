...
const string schema_path =
    "/Users/ihsuy/programming_pearls/column3_Data_Structures_Programs/"
    "schema_for_problem_6.txt";
...
string form_letter_generator(const vector<string>& field_values,
    ...
    for (int i = 0; i < schema.length(); ++i) {  // find field_values
        if (schema[i] == '$') {
            int field_begin = i;
            ...
            // fill in field value
            schema.replace(field_begin, field_index_str.length() + 1,
