...
struct Date
{
    ...
    {   // some date format checking (no rigirous)
        assert(d > 0 and d <= 31);
        assert(m > 0 and m <= 12);
...
int DaysPastInYear(const Date& date)
{   // next calculate number days from first day of date.year
    // to date.day
...
int DistanceBetweenDays(const Date& date1, const Date& date2)
{
    ...
    {   // if date1 and date2 are within the same year but not the same month
