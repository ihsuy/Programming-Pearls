...
#define INIT_TAX_GAP_GAP 5 // sequence: 70 75 80 85 .. which is 5(14+15+16+17...)
double CalculateTax(const double& income)
{
	...
	double affix_tax = tax_percentage*(income - (BASE_INCOME + INCOME_GAP*(income_level-1)));
	return prefix_tax + affix_tax;
}

// if there's gap and there's no patter for the the sequence 
// we can store the values in multiple arrays for each coefficient in the formula
// for example base_income{2200, 2700, 3200 ...} and so on
// then use a similar approach in the method above to first
// decide which income level the input has
// and use that income level as the key to retrieve all the coefficients
// the calculate the result
// there's no complicated conditional statement code block exists
