...
pair<int, int> decode1_2(
    const int& c) {  // 205/2048 is 0.10009765625; roughly 1/10
    ...
    for (int i = 0; i < nTest; ++i) {
        for (int a = 0; a <= 9; ++a) {
            for (int b = 0; b <= 9; ++b) {
                ...
                assert(d1.first == d1_2.first and d1.first == d1_3.first and
                       d1.first == d2.first);
                assert(d1.second == d1_2.second and d1.second == d1_3.second and
                       d1.second == d2.second);
            }
        }
    }
