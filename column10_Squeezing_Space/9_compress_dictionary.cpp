...
const string decoded_addr =
    "/Users/ihsuy/programming_pearls/decoded_engWords.txt";
const string compressed_addr =
    "/Users/ihsuy/programming_pearls/compressed_engWords.txt";
...
void DecodeDict() {
    ...
    for (int shift = 0; compressed_dict;) {
        ...
        if (BITS_IN_TOKEN + shift > BITS_IN_BYTE) {
            ...
            decoded_key |=
                ((encoded_byte & bitset<8>((1 << shift) - 1)) << offset);
        } else {
            shift += BITS_IN_TOKEN;
        }
