...
int fixedWindowSum(const vector<double>& nums, const int& m, const int& target = 0)
{   // some error handling code are expected here but I didnt write
    double window = 0.0;
    for (int i = 0; i < m; ++i)
    ...
    for (int i = 0, j = m; j < nums.size(); ++i, ++j)
    {   // slide while removing last number and adding next number
