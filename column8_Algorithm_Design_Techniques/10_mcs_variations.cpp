...
pair<int, int> SubvectorClosestToZero(
    const vector<int>&
        nums) {  // first calculate the cumulative sum by running through the
...
pair<int, int> SubvectorClosestTo(const int& N, const vector<int>& nums) {
    ...
    for (int i = 1; i < runningSums.size();
         ++i) {  // only part that's different from the previous solution
        // is here
