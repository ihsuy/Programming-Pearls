...
first block in the file, and to read the ith block, once you have read the i –
1st block. Reading the ith block from scratch therefore requires time
proportional to i. When Ed McCreight was designing a disk controller at Xerox
Palo Alto Research Center, he observed that by adding just one additional
pointer per node, you can keep all the other properties, but allow the ith block
to be read in time proportional to log i. How would you implement that insight?
*/
...
SeqFile* SearchFile(const int& id,
                    SeqFile* file_system,
