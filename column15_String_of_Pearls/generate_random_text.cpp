...
#define ALPHA_BASE 'a'
#define N_CHAR 27
vector<unordered_map<string, vector<int>>> LearnPatternMap(
    ...
    vector<unordered_map<string, vector<int>>> patternMaps(
        N, unordered_map<string, vector<int>>());
    patternMaps[0][" "] = vector<int>(N_CHAR, 0);

    string word;
    while (source >> word) {  // learn initials
...
string Order2RandomText(const int& nWords, const string& source_path) {
    ...
    vector<vector<vector<int>>> other_letters_dist(
        N_CHAR - 1, vector(N_CHAR - 1, vector<int>()));
    for (int i = 0; i < N_CHAR - 1; ++i) {
    ...
    string text;
    for (int i = 0; i < nWords; ++i) {  // generate initial letter
        string word;
...
int main() {
    const string source_path =
        "/Users/ihsuy/programming_pearls/shakespeare_small.txt";
