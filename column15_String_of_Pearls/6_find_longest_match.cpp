...
long long SearchInitial(char const** index_arr,
...
vector<pair<string, long long>> LongestMatch(const string& file_path,
                                             const vector<string>& queries,
                                             const int extra_range = 150) {
    char* buffer;
    ...
    for (int i = 0; i < query_size; ++i) {
        ...
        pair<long, long> pattern_pos =
            MatchPattern(index_arr, pos, fsize, queries[i]);
        responses.push_back(
            GetString(pattern_pos, buffer, index_arr, fsize, extra_range));
    }
...
void LongestMatch_app(
    const string& file_path,
    const int extra_range = 200) {  // load text file as a string of characters
    char* buffer;
    auto fsize = LoadFile(file_path, buffer);
