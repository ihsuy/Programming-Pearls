   ...
   public:
    IntSetBin(const int& maxelements, const int& maxval)
        : sentinel(new node(maxval, nullptr)),
          ...
          fn(0),
          shift(
              1) {  // calculate "shift" to replace division with bit operation
    ...
    void insert(const int& num) {  // hash
        int choice = num >> shift;

        if (bins[choice] == sentinel or
            bins[choice]->val > num) {  // use a node from free nodes
    ...
    bool find(const int& num) {  // a quick find function
