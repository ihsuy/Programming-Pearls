...
int main() {  // create test vector
    ...
    for (int i = 0, n = n_lower; i < sz; ++i) {
        ...
        if (rand() % 15) {  // occasionally introduce big gaps
            n += 10;
        }
        nums.push_back(n);
    }
    ...
    for (int i = 0; i < nTest; ++i) {
        ...
        // assert(res4.second < 0 or (nums[res4.second] == tar and
        // nums[res4.second - 1] != tar));
        assert(res5.second < 0 or
               (nums[res5.second] == tar and nums[res5.second - 1] != tar));
