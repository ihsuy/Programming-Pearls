...
int ssearch2(vector<int>& nums,
             const int& target,
             const int& size) {  // append the target value at the end of nums
    ...
    for (;; i++) {
        if (nums[i] == target) {  // this does 2 jobs: check "out of bound" and
                                  // "found the target"
            break;
        }
    }
    ...
    if (i == size) {  // for loop terminated because of out of bound?
        return -1;
    }
    // found
    return i;
