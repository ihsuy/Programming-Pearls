...
Assume that n is 10,000,000, and that the input file contains 1,000,000
integers.”
*/
...
bool isSortedAscend(
    const vector<int>& nums) {  // check if its sorted in ascending
...
vector<int> uniqueRandom(const int& k,
                         const int& n) {  // directly copied from problem 4
    if (k > n) {
...
void sys_sort(vector<int>& nums) {  // system sort wrapper
