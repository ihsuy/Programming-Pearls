...
struct defaultArray {  // data structure with every value set to a specific
                       // value
    ...
    int& operator[](const size_t& i) {
        if (from[i] >= top or
            to[from[i]] != i) {  // if this value has never been seen before
            // having from[i] < top makes sure that its possible
            // for i to be already in array "to"
            // but from[i] can also be < top by chance since part of it is
            // jumbled thats why we need to try "to[from[i]] == i" to make sure
            // that this i really exists in to
   ...
   private:
    int default_value;  // the value ith position should hold if its never
                        // touched by user
    int top;            // number of user set value
    int size_;          // total data size
    int* data;          // actual data
    int* from;          // maps from input index to array: "to"'s index
    int* to;  // an array where its 0~top th position stores indices for all
              // user touch datum in "data"
};
